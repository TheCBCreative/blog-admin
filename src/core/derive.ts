/**
 * Suggested excerpt and SEO fields derived from post content. These are editable
 * starting points shown to the author, not values applied silently.
 */
...
/**
 * Truncates without cutting mid-word, ending on a complete sentence when one
 * falls in the last 40% of the budget. Earlier sentence ends are ignored on
 * purpose: a body opening with "Hi." would otherwise yield a 3-character excerpt.
 */
...
function summarize(source: string, maxLength: number): string {
  const text = toPlainText(source);
  return text ? truncateNicely(text, maxLength) : '';
}
...
export function deriveSeoTitle(headline: string, maxLength = SEO_TITLE_MAX): string {
  return summarize(headline, maxLength);
}

/** Suggested meta description, from the excerpt if there is one, otherwise the body. */
export function deriveSeoDescription(
  opts: { excerpt?: string; body?: string },
  maxLength = SEO_DESCRIPTION_MAX,
): string {
  return summarize(opts.excerpt?.trim() || opts.body || '', maxLength);
}
